...
qual_cuts_g = ((R['extended_class_g'] == 0) | (R['extended_class_g'] == 1)) & (R['flags_g'] < 4) & (R['magerr_auto_g'] < 0.2)
qual_cuts_r = ((R['extended_class_r'] == 0) | (R['extended_class_r'] == 1)) & (R['flags_r'] < 4) & (R['magerr_auto_r'] < 0.2)
qual_cuts_i = ((R['extended_class_i'] == 0) | (R['extended_class_i'] == 1)) & (R['flags_i'] < 4) & (R['magerr_auto_i'] < 0.2)
...
qual_cuts = qual_cuts_g & qual_cuts_r & qual_cuts_i
...
dist_mods = np.arange(14, 18, 0.1)
sel = mv.isochrone_selection(R, distance_modulus = dist_mod)
#hpxcube = mv.create_hpxcube(R, mv.isochrone_selection, \
#                           dist_mods, nside=64)

img_names = []
for i in range(len(dist_mods)):
    sel = mv.isochrone_selection(R, distance_modulus = dist_mods[i])
    
    fig, axarr = plt.subplots(1, 2, figsize = (15, 8))
    
    axarr[0].plot((R['gmag'] - R['rmag'])[choose2 & sel], R['gmag'][choose2 & sel], '.k', zorder =0)
    axarr[0].plot((R['gmag'] - R['rmag'])[choose2 & sel], R['gmag'][choose2 & sel], '.b', zorder =1)
    yrange = np.arange(16, 25, 0.1)
    axarr[0].plot(iso_model(yrange), yrange, '--r', linewidth=3)
    axarr[0].set_xlabel('g-r')
    axarr[0].set_ylabel('g')
    axarr[0].set_xlim([-0.50, 1])
    axarr[0].set_ylim([14, 23])
    axarr[0].invert_yaxis()
    
    axarr[1].hist2d(R['ra'][sel], R['dec'][sel], bins =50, vmin = 0, vmax = 1000)
    axarr[1].set_xlabel('RA')
...
mv.hollywood(img_names, outfile = 'test_isochroneselection.gif')

#Select using a box
dist_mods = np.arange(14, 18, 0.1)
sel = mv.isochrone_selection(R, distance_modulus = dist_mod)
#hpxcube = mv.create_hpxcube(R, mv.box_selection, \
#                           dist_mods, nside=64)

#Save frames
img_names = []
for i in range(len(dist_mods)):
    sel = mv.box_selection(R, distance_modulus = dist_mods[i])
    fig, axarr = plt.subplots(1, 2, figsize = (15, 8))
    ...
    axarr[1].hist2d(R['ra'][sel], R['dec'][sel], bins =50)
    axarr[1].set_xlabel('RA')
    axarr[1].set_ylabel('DEC')
    plt.savefig('image' + str(i) + '.png')
    plt.close()
...
mv.hollywood(img_names, outfile = 'test_boxselection.gif')
